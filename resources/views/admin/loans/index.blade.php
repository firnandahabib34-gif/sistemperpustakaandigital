        ...
        <tbody class="divide-y">
            ...
            <tr>
                ...
                <td class="px-6 py-3 text-center whitespace-nowrap">
                    ...
                    @elseif($loan->status == 'menunggu_validasi')
                        <form method="POST" action="{{ route('admin.loans.return', $loan->id) }}" class="inline" onsubmit="return confirm('Yakin buku sudah dikembalikan secara fisik?')">
                            @csrf
                            @method('PATCH')
                            <button class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm">Konfirmasi Pengembalian</button>
                        </form>
                    @else
